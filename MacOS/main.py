...
n = 0
#lock prevents more than 2 cards being flipped at once
lock = False  
...
class UI:
    ...
    def start_game(self, player_name, name_window):
        ...
        class Memory_Game:
            ...
            def f(self):
                global l1, l2, l3, n, lock
                if self.button not in l2:
                    if lock or self.button in l2:
                        return
                    # flip this button
                    l1.append(self.text)
                    ...
                    # if two cards are now face-up, schedule the compare
                    if len(l1) == 2:
                        lock = True
                        # force a redraw of both cards immediately
                        for btn in l2:
                            btn.update_idletasks()

                        # after 800ms, call a helper to evaluate
                        self.button.after(800, self._evaluate)

            def _evaluate(self):
                """Called 800ms after the second card flips up."""
                global l1, l2, l3, n, lock

                # mismatch?
                if l1[0] != l1[1]:
                    for btn in l2:
                        btn.configure(image=backbutton)    # flip both back
                    n += 1
                else:
                    self.match_sound.play()
                    for btn in l2:
                        btn.configure(command=lambda: None)  # disable matched
                        l3.append(btn)
                    n += 1
